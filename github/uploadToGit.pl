...
my $prepareForPullRequest = 0;                                                  # Prepare for pull request if true
#my $generateDocumentation = 0;                                                  # Generate and upload documentation from local computer if true

my $home = q(/home/phil/vita/raysect/source);                                   # Home folder
...
if (1)                                                                          # Commit to vita repository
 {#for my $file(searchDirectoryTreesForMatchingFiles($out, $docs))               # Remove generated files
  ...
  my @f = searchDirectoryTreesForMatchingFiles($home);                          # Files we want to upload
  for my $f(@f)
   {lll qx(git add $f);
   }

  my $title = q(Raysect).dateTimeStampName;                                     # Name for commit
